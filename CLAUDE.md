...
- `RESPONSE_TO_REVIEWERS.pdf` is built by `scripts/make_response_letter.sh` (pandoc + pdflatex), not hand-rendered. `latex/response_letter_style.tex` makes the PDF look like a markdown preview: referee blockquotes in muted gray behind a light gray left bar, underlined referee headings. Headings and their underlines are boxed together and given `\needspace` so page breaks cannot orphan either one; keep those guards if editing the style.
